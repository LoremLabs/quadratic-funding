import { calculateQf } from "./index";

describe("It should throw errors if the input is invalid", () => {
  test("should throw an error if projects is not an array", () => {
    expect(() => calculateQf({} as any, 0)).toThrow(
      "projects must be an array"
    );
  });

  test("should throw an error if matchingAmount is not provided", () => {
    expect(() => calculateQf([], undefined as any)).toThrow(
      "matchingAmount must be provided"
    );
  });

  test("should throw an error if matchingAmount is less than 0", () => {
    expect(() => calculateQf([], -1)).toThrow(
      "matchingAmount must be greater or equal to 0"
    );
  });

  test("should throw an error if project does not contain an identifier", () => {
    expect(() => calculateQf([{ amount: 0, match: 0 } as any], 0)).toThrow(
      "project must contain a defined identifier"
    );
  });

  test("should throw an error if project contains an empty identifier", () => {
    expect(() =>
      calculateQf([{ amount: 0, match: 0, identifier: "" } as any], 0)
    ).toThrow("project must contain an identifier");
  });

  test("should throw an error if project contains a negative amount", () => {
    expect(() =>
      calculateQf(
        [{ identifier: "abc", match: 0, contributions: [{ amount: -1 }] }],
        0
      )
    ).toThrow("contribution amount must be greater than 0");
  });
...
describe("It should calculateQf for a set of example qfTestCases", () => {
  ...
  qfTestCases.forEach((testCase) => {
    ...
    test(testCase.name, () => {
      expect(calculateQf(projects, matchingAmount)).toEqual(expected);
    });
  });
});
